#!/usr/bin/env bash
# deploy-otel.sh — Deploy the production OpenTelemetry Collector stack (helios-cli)
...
# ── Pull images ────────────────────────────────────────────────────
echo "[helios-otel] Pulling images …"
docker compose -f "${COMPOSE_FILE}" pull --quiet

# ── Start stack ────────────────────────────────────────────────────
echo "[helios-otel] Starting collector stack in background …"
...
echo "[helios-otel] Waiting for collector health (timeout ${TIMEOUT}s) …"
while (( ELAPSED < TIMEOUT )); do
    if curl -sf "${HEALTH_URL}" &>/dev/null; then
        echo "[helios-otel] Collector is healthy."
        break
    fi
    sleep "${INTERVAL}"
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  helios-cli OpenTelemetry Collector — Endpoints"
echo "═══════════════════════════════════════════════════════════"
...
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "[helios-otel] Deployment complete."
