...
This runbook explains how to configure, operate, and respond to SLO burn rate alerts for the Helios CLI repository.

## Overview

The SLO alerting system monitors CI/CD pipeline health against a **99.0% success rate** SLO target. It consists of two workflows:
...
|-----------|-------|-------------|
| SLO Target | 99.0% | Required CI success rate |
| Error Budget | 1.0% | Allowable failure rate |
| Monthly Budget | 432 minutes | Maximum allowed downtime per month |
...
curl -X POST -H 'Content-type: application/json' \
  --data '{"text":"Test SLO alert from helios-cli"}' \
  YOUR_WEBHOOK_URL
```
...
curl -X POST https://events.pagerduty.com/v2/enqueue \
  ...
  -d '{
    ...
    "payload": {
      "summary": "Test SLO alert from helios-cli",
