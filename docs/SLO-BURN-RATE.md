# SLO Burn Rate Alerting — Helios CLI

> **Status:** Active
> **Owner:** Helios CLI SRE / maintainers
...
Helios CLI CI/CD pipeline to detect SLO violations early.
...
| Parameter | Value |
|-----------|-------|
| **SLO Target** | 99.0% CI success rate |
| **Monthly Error Budget** | 7.2 hours (~0.01 × 30 days × 24 h) |
| **Budget Expressed As** | 7.2 hours of CI failure time per 30-day window |
| **Measurement** | CI workflow run success/failure on `main` branch |

### Why 99.0%?

Helios CLI is a developer-facing tool with a broad CI matrix (multiple
platforms, Bazel and Cargo builds, fuzzing). A 99.0% SLO balances reliability
with the inherent complexity of the build system while still catching
...
For a 99.0% SLO:
- A 1x burn rate means `1.0%` failures in the window (healthy)
- A 14.4x burn rate means `14.4%` failures in the window (budget exhausts in 2 days)
...
| Period | Total Minutes | 1.0% Budget | Budget Consumed |
|--------|---------------|-------------|-----------------|
| 1 day | 1,440 min | 14.4 min | Measured by workflow |
| 7 days | 10,080 min | 100.8 min | Measured by workflow |
| 30 days | 43,200 min | 432 min (7.2 hr) | Measured by workflow |
