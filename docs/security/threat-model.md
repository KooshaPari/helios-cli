...
| **E — Elevation**       | high   | A workflow inherits broad repository permissions or a compromised cross-repository token | Repository workflow permissions currently default to `write`; 12 of 26 workflow files now declare a top-level `permissions:` block, including `ci.yml`, `rust-ci.yml`, `rust-ci-full.yml`, `rust-ci-full-nextest-platform.yml`, `sdk.yml`, `blob-size-policy.yml`, and `python-runtime-build.yml` with only `contents: read`. The built-in token remains limited to this repository and cannot authorize upstream artifact downloads                                                                                                                                                                                                                     | ci-ops   | 2026-07-18    |
