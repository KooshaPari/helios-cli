...
async def _call_direct_minimax(
    ...
    payload = {"model": mm_model, "messages": [{"role": "user", "content": prompt}]}
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        for i in range(num_trials):
            ...
            if num_agents > 1:
                ...
                tasks = [_call_llm(client, model, prompt) for _ in range(num_agents)]
...
def compare_results(results: dict[str, ExtendedBenchmarkResult]) -> None:
