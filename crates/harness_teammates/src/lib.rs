...
impl TeammateRegistry {
    pub fn new() -> Self {
        Self { teammates: RwLock::new(HashMap::new()) }
    }
    ...
    pub fn list(&self) -> Vec<Teammate> {
        self.teammates.read().ok().map(|m| m.values().cloned().collect()).unwrap_or_default()
    }
    ...
    pub fn unregister(&self, id: &str) -> bool {
        self.teammates.write().ok().map(|mut m| m.remove(id).is_some()).unwrap_or(false)
    }
}
