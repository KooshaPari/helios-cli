...
impl SpecGenerator {
    ...
    #[instrument(skip(self, intent), fields(intent = ?intent.intent, confidence = intent.confidence))]
    pub fn generate(&self, intent: &ClassifiedIntent) -> Result<Specification> {
        ...
        debug!(name = %spec.spec.name, "generated spec");
        Ok(spec)
    }
    ...
    fn generate_verification(&self, intent: &ClassifiedIntent) -> Vec<VerificationRule> {
        intent
            ...
            .map(|v| match *v {
                "test" => {
                    VerificationRule::Test { name: "unit_tests".to_string(), timeout_seconds: 300 }
                }
                ...
                "smoke_test" => {
                    VerificationRule::Test { name: "smoke_tests".to_string(), timeout_seconds: 60 }
                }
                ...
                _ => VerificationRule::Test { name: v.to_string(), timeout_seconds: 300 },
            })
            .collect()
    }
    ...
    fn generate_behavior(&self, intent: &ClassifiedIntent) -> BehaviorSpec {
        ...
        BehaviorSpec { given, when: when.to_string(), then: then.to_string(), and: vec![] }
    }
...
impl ElicitationHandler {
    /// Create new handler
    pub fn new() -> Self {
        Self { classifier: IntentClassifier::new(), generator: SpecGenerator::new() }
    }
