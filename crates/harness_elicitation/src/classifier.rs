...
impl IntentClassifier {
    ...
    pub fn classify(&self, input: &str) -> Result<ClassifiedIntent> {
        ...
        // Find best match
        let best = scores.iter().max_by(|a, b| a.1.total_cmp(b.1)).map(|(i, s)| (i.clone(), *s));
        ...
        let max_score = self.patterns.get(&intent).map(|r| r.len() as f64).unwrap_or(1.0);
        let confidence = (score / max_score).min(1.0);

        // Extract entities
        let entities = self.extract_entities(input);

        debug!(?intent, confidence, entities = entities.len(), "classified intent");
        Ok(ClassifiedIntent { intent, confidence, entities, original_input: input.to_string() })
    }
...
mod tests {
    ...
    fn test_classify_fix() {
        ...
        let result = classifier.classify("Fix the auth bug in login").expect("classify failed");
    ...
    fn test_classify_feature() {
        let classifier = IntentClassifier::new();
        let result = classifier.classify("Add OAuth login support").expect("classify failed");
    ...
    fn test_classify_refactor() {
        let classifier = IntentClassifier::new();
        let result = classifier.classify("Refactor the user service").expect("classify failed");
