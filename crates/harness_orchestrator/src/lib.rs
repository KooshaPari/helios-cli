...
impl RootManager {
    ...
    pub async fn register_agent(&self, name: &str, caps: Vec<AgentCapability>) -> Uuid {
        ...
        debug!(agent_id = %id, %name, "registered agent");
        id
    }

    #[instrument(skip(self), fields(spec_name = %spec_name))]
    pub async fn decompose(&self, spec_name: &str) -> Vec<Task> {
        ...
        debug!(count = tasks.len(), "decomposed spec into tasks");
        tasks
    }
    ...
    pub async fn execute(&self) -> Vec<String> {
        ...
        while let Some(mut task) = self.tasks.write().await.pop_front() {
            ...
            // Release agent
            if let Some(a) = self.agents.write().await.iter_mut().find(|a| a.id == agent_id) {
                a.release(true);
            }
        }

        debug!(results = results.len(), "execute() finished");
        results
    }
