...
#[instrument(skip(spec), fields(name = %spec.spec.name))]
...
fn validate_version(version: &str) -> Result<()> {
    ...
    parts[0].parse::<u32>().map_err(|_| SpecError::InvalidValue {
        field: "version".to_string(),
...
pub fn validate_with_options(spec: &Specification, options: &ParseOptions) -> Result<()> {
    ...
    if options.strict {
        ...
        if spec.spec.owner.is_empty() {
            return Err(SpecError::ValidationError("Owner is required in strict mode".to_string()));
        }
