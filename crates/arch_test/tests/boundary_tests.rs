...
fn domain_has_no_external_dependencies() {
    ...
    let forbidden_deps = ["tokio", "serde_json", "reqwest", "sqlx", "tracing"];

    for entry in WalkDir::new(domain_path).into_iter().filter_map(|e| e.ok()) {
...
fn adapters_only_depend_on_domain() {
    ...
    for entry in WalkDir::new(adapters_path).into_iter().filter_map(|e| e.ok()) {
        if entry.path().extension().map(|e| e == "rs").unwrap_or(false) {
            let content = std::fs::read_to_string(entry.path()).unwrap_or_default();
...
fn no_cyclic_dependencies() {
    ...
    for crate_dir in &crate_dirs {
        ...
        if path.exists() {
            assert!(
                !has_cycle(path, &mut visited, &mut stack),
                "Cyclic dependency found in: {}",
                crate_dir
            );
        }
    }
...
fn hexagonal_structure_followed() {
    ...
    let structure_valid = expected_modules
        .iter()
        .all(|m| Path::new(m).exists() || Path::new(&format!("src/{}", m)).exists());
