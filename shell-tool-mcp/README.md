...
Because `@phenotype/helios-shell-tool-mcp` intercepts `execve(2)` calls directly, it _always_ knows the full path to the program being executed. In turn, this makes it possible to provide stronger guarantees on how [Helios `.rules`](https://developers.openai.com/codex/local-config#rules-preview) are enforced.
...
```bash
npx -y @phenotype/helios-shell-tool-mcp --version
```

To test out the MCP with a one-off invocation of Helios CLI, it is important to _disable_ the default shell tool in addition to enabling the MCP so Helios has exactly one shell-like tool available to it:

```bash
codex --disable shell_tool \
  --config 'mcp_servers.bash={command = "npx", args = ["-y", "@phenotype/helios-shell-tool-mcp"]}'
```
...
command = "npx"
args = ["-y", "@phenotype/helios-shell-tool-mcp"]
```

Note when the `@phenotype/helios-shell-tool-mcp` launcher runs, it selects the appropriate native binary to run based on the host OS/architecture. For the Bash wrapper, it inspects `/etc/os-release` on Linux or the Darwin major version on macOS to try to find the best match it has available. See [`bashSelection.ts`](https://github.com/openai/codex/blob/main/shell-tool-mcp/src/bashSelection.ts) for details.

## MCP Client Requirements

This MCP server is designed to be used with [Helios](https://developers.openai.com/codex/cli), as it declares the following `capability` that Helios supports when acting as an MCP client:
...
The Helios harness (used by the CLI and the VS Code extension) sends such requests to MCP servers that declare the `codex/sandbox-state` capability.
...
See [the README in the Helios repo](https://github.com/openai/codex/blob/main/codex-rs/shell-escalation/README.md) for details.
