...
pub enum Command {
    /// Submit a new Helios Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Helios Cloud task.
    Status(StatusCommand),
    /// List Helios Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Helios Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Helios Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Helios Cloud.
    ...
    /// Git branch to run in Helios Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Helios Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Helios Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Helios Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
