...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"helios-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: helios_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(&self) -> Option<ConfigRequirementsToml> {
        let _timer =
            helios_otel::start_global_timer("codex.cloud_requirements.fetch.duration_ms", &[]);
...
mod tests {
    ...
    async fn fetch_cloud_requirements_writes_cache_when_identity_is_incomplete() {
        ...
        let path = helios_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
