...
const X_HELIOS_TURN_STATE_HEADER: &str = "x-helios-turn-state";
...
async fn connect_websocket(
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_HELIOS_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-helios-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-helios-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
