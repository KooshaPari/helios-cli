...
# Run a suite of smoke tests against the Windows sandbox via the Helios CLI
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the Helios CLI to invoke `codex sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Helios.
    """
    ...
    raise FileNotFoundError(
        "Helios CLI not found. Build it first, e.g.\n"
