...
/// Special argv[1] flag used when the Helios executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `helios-apply-patch` (to avoid forcing
/// `helios-arg0` to depend on `helios-core`), it is part of the "codex core"
/// process-invocation contract between the apply-patch runtime and the arg0
/// dispatcher.
pub const HELIOS_CORE_APPLY_PATCH_ARG1: &str = "--helios-run-as-apply-patch";
