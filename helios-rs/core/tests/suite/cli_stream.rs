...
use helios_protocol::protocol::GitInfo;
...
fn repo_root() -> std::path::PathBuf {
    ...
    helios_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = helios_utils_cargo_bin::cargo_bin("codex").unwrap();
...
/// received by a mock Phenotype Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Helios to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// 2. Configures codex to read from this fixture via HELIOS_RS_SSE_FIXTURE env var
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = helios_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("HELIOS_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("HELIOS_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume helios-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = helios_core::git_info::collect_git_info(&git_repo).await;
