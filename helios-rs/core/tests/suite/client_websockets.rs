...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-helios-primary-used-percent": "100.0",
            "x-helios-secondary-used-percent": "87.5",
            "x-helios-primary-over-secondary-limit-percent": "95.0",
            "x-helios-primary-window-minutes": "15",
            "x-helios-secondary-window-minutes": "60"
        }
    });
...
async fn responses_websocket_forwards_turn_metadata_on_create_and_append() {
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-helios-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.append"));
    assert_eq!(
        second["client_metadata"]["x-helios-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn websocket_harness_with_options(
    ...
    let mut model_info = helios_core::test_support::construct_model_info_offline(MODEL, &config);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "helios-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
