...
use helios_core::compact::SUMMARY_PREFIX;
...
async fn assert_compaction_uses_turn_lifecycle_id(codex: &std::sync::Arc<helios_core::CodexThread>) {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        helios_protocol::models::ResponseItem::Message {
            id: None,
        ...
        helios_protocol::models::ResponseItem::Compaction {
...
async fn snapshot_request_shape_pre_turn_compaction_strips_incoming_model_switch() {
    ...
    let server = start_mock_server().await;
    let previous_model = "gpt-5.1-helios-max";
    ...
    let test = test_codex()
        ...
        .with_config(move |config| {
            ...
            config
                .features
                .enable(helios_core::features::Feature::RemoteModels);
            config.model_auto_compact_token_limit = Some(200);
        })
        .build(&server)
        .await
