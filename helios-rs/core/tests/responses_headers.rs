...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let helios_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-helios-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        .single_request()
        .header("x-helios-turn-metadata")
        .expect("x-helios-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-helios-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-helios-turn-metadata"
    );
    assert_eq!(
        initial_parsed
            .get("sandbox")
