...
pub mod test_codex;
pub mod test_helios_exec;

#[ctor]
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    helios_core::test_support::set_thread_manager_test_mode(true);
    helios_core::test_support::set_deterministic_process_ids(true);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = helios_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("helios-rs"));
...
pub async fn load_default_config_for_test(helios_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        helios_linux_sandbox_exe: Some(
            helios_utils_cargo_bin::cargo_bin("helios-linux-sandbox")
                .expect("should find binary for helios-linux-sandbox"),
        ),
...
pub async fn wait_for_event<F>(
    codex: &CodexThread,
    predicate: F,
) -> helios_protocol::protocol::EventMsg
where
    F: FnMut(&helios_protocol::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&helios_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    helios_core::spawn::HELIOS_SANDBOX_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    helios_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    helios_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    helios_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
