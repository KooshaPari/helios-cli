...
mod tests {
    ...
    async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            "export HELIOS_INTERACTIVE_SHELL_VAR=codex\n",
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "echo $HELIOS_INTERACTIVE_SHELL_VAR\n",
            2_500,
        )
        .await?;
        assert!(
    ...
    async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
        ...
        let out_2 =
            exec_command(&session, &turn, "echo $HELIOS_INTERACTIVE_SHELL_VAR", 2_500).await?;
    ...
    async fn unified_exec_timeouts() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            format!("export HELIOS_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "sleep 5 && echo $HELIOS_INTERACTIVE_SHELL_VAR\n",
            10,
        )
        .await?;
        assert!(
