...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == HELIOS_APPS_MCP_SERVER_NAME {
    ...
    if server == HELIOS_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
    ...
    .await
    {
        let result = match decision {
            McpToolApprovalDecision::Accept | McpToolApprovalDecision::AcceptAndRemember => {
                ...
                .await;
                maybe_track_helios_app_used(sess.as_ref(), turn_context, &server, &tool_name).await;
                result
            }
...
async fn maybe_track_helios_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != HELIOS_APPS_MCP_SERVER_NAME {
        return;
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    let approval_key = if approval_mode == AppToolApproval::Auto {
        ...
        if server == HELIOS_APPS_MCP_SERVER_NAME && connector_id.is_none() {
            None
        } else {
...
mod tests {
    ...
    #[test]
    fn helios_apps_tool_question_keeps_legacy_app_label() {
        let question = build_mcp_tool_approval_question(
            "q".to_string(),
