        ...
        config_layer_stack.get_layers(ConfigLayerStackOrdering::HighestPrecedenceFirst, true)
    {
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$HELIOS_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                ...
                // Embedded system skills are cached under `$HELIOS_HOME/skills/.system` and are a
...
mod tests {
    ...
    async fn make_config(helios_home: &TempDir) -> Config {
        make_config_for_cwd(helios_home, helios_home.path().to_path_buf()).await
    }

    async fn make_config_for_cwd(helios_home: &TempDir, cwd: PathBuf) -> Config {
        let trust_root = cwd
            .ancestors()
    ...
    fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_helios_folder: project_dot_codex,
                },
                TomlValue::Table(toml::map::Map::new()),
                "marked untrusted",
            ),
    ...
    fn write_skill(helios_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&helios_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &helios_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skill_dependencies_metadata_from_yaml() {
        ...
        let skill_path = write_skill(&helios_home, "demo", "dep-skill", "from json");
...
"#,
        ...
        let cfg = make_config(&helios_home).await;
    ...
    async fn loads_skill_interface_metadata_from_yaml() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "ui-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"##,
    ...
    async fn loads_skill_policy_from_yaml() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "policy-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "policy-empty", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skill_permissions_from_yaml() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "permissions-skill", "from yaml");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn empty_skill_permissions_do_not_create_profile() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "permissions-empty", "from yaml");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skill_macos_permissions_from_yaml() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "permissions-macos", "from yaml");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        ...
        fs::create_dir_all(helios_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &helios_home.path().join("skills/shared"));
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        // Create a cycle:
        //   $HELIOS_HOME/skills/cycle/loop -> $HELIOS_HOME/skills/cycle
        let cycle_dir = helios_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&helios_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills_for_test(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = helios_home.path().join("skills/.system");
    ...
    async fn respects_max_scan_depth_for_user_scope() {
        ...
        let skills_root = helios_home.path().join("skills");
        let outcome = load_skills_from_roots([SkillRoot {
            path: skills_root,
            scope: SkillScope::User,
        }]);
    ...
    async fn loads_valid_skill() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&helios_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&helios_home).await;
    ...
    async fn skips_hidden_and_invalid() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = helios_home.path().join("skills/.hidden");
        ...
        let invalid_dir = helios_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&helios_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&helios_home).await;
        ...
        write_skill(&helios_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills_for_test(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    #[tokio::test]
    async fn loads_skills_from_agents_dir_without_helios_dir() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    #[tokio::test]
    async fn loads_skills_from_all_helios_dirs_under_project_root() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
        ...
        let cfg = make_config_for_cwd(&helios_home, nested_dir).await;
    ...
    #[tokio::test]
    async fn loads_skills_from_helios_dir_when_not_git_repo() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        ...
        let cfg = make_config_for_cwd(&helios_home, work_dir.path().to_path_buf()).await;
    ...
    async fn keeps_duplicate_names_from_repo_and_user() {
        ...
        let user_skill_path = write_skill(&helios_home, "user", "dupe-skill", "from user");
    ...
    #[tokio::test]
    async fn keeps_duplicate_names_from_nested_helios_dirs() {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&helios_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&helios_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&helios_home, "system", "system-skill", "from system");
