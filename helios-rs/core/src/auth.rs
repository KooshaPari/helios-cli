...
use helios_app_server_protocol::AuthMode as ApiAuthMode;
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "HELIOS_REFRESH_TOKEN_URL_OVERRIDE";
...
impl CodexAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(helios_home.to_path_buf(), storage_mode);
    ...
    pub fn from_auth_storage(
        ...
        load_auth(helios_home, false, auth_credentials_store_mode)
    }
...
pub const HELIOS_API_KEY_ENV_VAR: &str = "HELIOS_API_KEY";
...
pub fn read_helios_api_key_from_env() -> Option<String> {
    env::var(HELIOS_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `helios_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    helios_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(helios_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(helios_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(helios_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(helios_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(helios_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(helios_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

fn load_auth(
    helios_home: &Path,
    enable_helios_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<CodexAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        let client = crate::default_client::create_client();
        CodexAuth::from_auth_dot_json(helios_home, auth_dot_json, storage_mode, client)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_helios_api_key_env && let Some(api_key) = read_helios_api_key_from_env() {
        let client = crate::default_client::create_client();
...
impl AuthManager {
    ...
    pub(crate) fn from_auth_for_testing(auth: CodexAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            helios_home: PathBuf::from("non-existent"),
            inner: RwLock::new(cached),
            enable_helios_api_key_env: false,
    ...
    fn load_auth_from_storage(&self) -> Option<CodexAuth> {
        load_auth(
            &self.helios_home,
            self.enable_helios_api_key_env,
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.helios_home, self.auth_credentials_store_mode)?;
...
mod tests {
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let auth = super::load_auth(helios_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, helios_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(helios_home);
    ...
    async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
        let _guard = EnvVarGuard::set(HELIOS_API_KEY_ENV_VAR, "sk-env");
        let helios_home = tempdir().unwrap();
