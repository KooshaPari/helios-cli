...
impl ModelsManager {
    ...
    ///
    /// Uses `helios_home` to store cached model metadata and initializes with built-in presets.
    ...
    pub fn new(
        ...
        let cache_path = helios_home.join(MODEL_CACHE_FILE);
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            helios_otel::start_global_timer("codex.remote_models.fetch_update.duration_ms", &[]);
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            helios_otel::start_global_timer("codex.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::models_manager::client_version_to_whole();
...
mod tests {
    ...
    async fn get_model_info_tracks_fallback_usage() {
        let helios_home = tempdir().expect("temp dir");
        ...
        let manager = ModelsManager::new(helios_home.path().to_path_buf(), auth_manager, None);
