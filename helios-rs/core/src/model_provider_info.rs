...
use helios_api::Provider as ApiProvider;
use helios_api::provider::RetryConfig as ApiRetryConfig;
use http::HeaderMap;
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These HELIOS_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_helios_oss_base_url = format!(
        "http://localhost:{helios_oss_port}/v1",
        helios_oss_port = std::env::var("HELIOS_OSS_PORT")
            .ok()
    ...
    let helios_oss_base_url = std::env::var("HELIOS_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_helios_oss_base_url);
    create_oss_provider_with_base_url(&helios_oss_base_url, wire_api)
}
