...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .helios_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = helios_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        helios_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = helios_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = helios_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when HELIOS_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let helios_home = tmp.path().join("home");
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    let helios_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&helios_home).await?;
    make_config_for_test(&helios_home, &project_root, TrustLevel::Trusted, None).await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_helios_folder } => Some(dot_helios_folder),
            _ => None,
        })
        .collect();
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_helios_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let helios_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&helios_home).await?;
    tokio::fs::write(helios_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn helios_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let helios_home = home_dir.join(".codex");
    tokio::fs::create_dir_all(&helios_home).await?;
    tokio::fs::write(helios_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn helios_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_helios_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let helios_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&helios_home_untrusted).await?;
    make_config_for_test(
        &helios_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = helios_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let helios_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&helios_home_unknown).await?;
    tokio::fs::write(
        helios_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let helios_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&helios_home).await?;
        let config_path = helios_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&helios_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use helios_config::RequirementsExecPolicyDecisionToml;
    use helios_config::RequirementsExecPolicyParseError;
    use helios_config::RequirementsExecPolicyPatternTokenToml;
    use helios_config::RequirementsExecPolicyPrefixRuleToml;
    use helios_config::RequirementsExecPolicyToml;
    ...
    fn config_stack_for_dot_helios_folder_with_requirements(
        dot_helios_folder: &Path,
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_helios_folder_with_requirements(temp_dir.path(), requirements);
