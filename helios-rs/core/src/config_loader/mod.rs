...
pub use helios_config::CloudRequirementsLoader;
pub use helios_config::ConfigError;
pub use helios_config::ConfigLayerEntry;
pub use helios_config::ConfigLayerStack;
pub use helios_config::ConfigLayerStackOrdering;
pub use helios_config::ConfigLoadError;
pub use helios_config::ConfigRequirements;
pub use helios_config::ConfigRequirementsToml;
pub use helios_config::ConstrainedWithSource;
pub use helios_config::LoaderOverrides;
pub use helios_config::McpServerIdentity;
pub use helios_config::McpServerRequirement;
pub use helios_config::NetworkConstraints;
pub use helios_config::NetworkRequirementsToml;
pub use helios_config::RequirementSource;
pub use helios_config::ResidencyRequirement;
pub use helios_config::SandboxModeRequirement;
pub use helios_config::Sourced;
pub use helios_config::TextPosition;
pub use helios_config::TextRange;
pub use helios_config::WebSearchModeRequirement;
pub(crate) use helios_config::build_cli_overrides_layer;
pub(crate) use helios_config::config_error_from_toml;
pub use helios_config::format_config_error;
pub use helios_config::format_config_error_with_source;
pub(crate) use helios_config::io_error_from_config_error;
pub use helios_config::merge_toml_values;
#[cfg(test)]
pub(crate) use helios_config::version_for_toml;
...
pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    helios_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}

pub(crate) async fn first_layer_config_error_from_entries(
    layers: &[ConfigLayerEntry],
) -> Option<ConfigError> {
    helios_config::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE)
        .await
}
...
pub async fn load_config_layers_state(
    ...
    // Add a layer for $HELIOS_HOME/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, helios_home)?;
...
#[cfg(windows)]
fn windows_helios_system_dir() -> PathBuf {
...
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_helios_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_helios_system_dir().join("config.toml");
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_helios_folder: dot_helios_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let helios_home_abs = AbsolutePathBuf::from_absolute_path(helios_home)?;
    let helios_home_normalized =
        normalize_path(helios_home_abs.as_path()).unwrap_or_else(|_| helios_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        ...
        let config_file = dot_helios_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_helios_abs.as_path())?;
                let entry =
                    project_layer_entry(trust_context, &dot_helios_abs, &layer_dir, config, true);
                layers.push(entry);
            }
            Err(err) => {
