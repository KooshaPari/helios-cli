...
impl RolloutRecorder {
    ...
    async fn list_threads_with_db_fallback(
        ...
        let fs_page = if archived {
            let root = helios_home.join(ARCHIVED_SESSIONS_SUBDIR);
            get_threads_in_root(
                root,
                fs_page_size,
                cursor,
...
fn precompute_log_file_info(
    ...
    let mut dir = config.helios_home.clone();
...
impl From<helios_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: helios_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
...
mod tests {
    ...
    async fn list_threads_db_enabled_drops_missing_rollout_paths() -> std::io::Result<()> {
        ...
        let mut builder = helios_state::ThreadMetadataBuilder::new(
            thread_id,
            stale_path,
            created_at,
            SessionSource::Cli,
