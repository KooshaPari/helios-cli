...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in HELIOS_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in HELIOS_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Expected structure for $HELIOS_HOME/auth.json.
...
pub(super) fn get_auth_file(helios_home: &Path) -> PathBuf {
    helios_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(helios_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(helios_home: PathBuf) -> Self {
        Self { helios_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `HELIOS_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.helios_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.helios_home)
    }
}
...
impl KeyringAuthStorage {
    fn new(helios_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            helios_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.helios_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.helios_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.helios_home)?;
...
impl AutoAuthStorage {
    fn new(helios_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(helios_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(helios_home)),
        }
    }
}
...
// A global in-memory store for mapping helios_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(helios_home: PathBuf) -> Self {
        Self { helios_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(helios_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(helios_home)),
...
mod tests {
    ...
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        ...
        let storage = FileAuthStorage::new(helios_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(helios_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(helios_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let helios_home = PathBuf::from("~/.codex");

        let key = compute_store_key(helios_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(helios_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(helios_home.path())?;
