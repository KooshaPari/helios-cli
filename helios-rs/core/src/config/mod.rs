...
use helios_app_server_protocol::Tools;
use helios_app_server_protocol::UserSavedConfig;
use helios_protocol::config_types::AltScreenMode;
...
pub mod service;
pub mod types;
pub use helios_config::Constrained;
pub use helios_config::ConstraintError;
pub use helios_config::ConstraintResult;
...
pub use helios_git::GhostSnapshotConfig;
...
pub(crate) const DEFAULT_AGENT_MAX_SPAWN_DEPTH: i32 = DEFAULT_AGENT_MAX_DEPTH;
...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`Helios <noreply@openai.com>`)
    /// - `Some("")` or whitespace-only: disable commit attribution
    /// - `Some("...")`: use the provided attribution text verbatim
    pub commit_attribution: Option<String>,

    /// Optional external notifier command. When set, Helios will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Helios
    ...
    ///
    /// ```toml
    /// notify = ["notify-send", "Helios"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Helios '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Helios home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Helios can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Helios unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/helios-rs/rmcp-client/src/oauth.rs#L2
    /// file: HELIOS_HOME/.credentials.json
    ///       This file will be readable to Helios and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Helios will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Helios state (defaults to `~/.codex` but can be
    /// overridden by the `HELIOS_HOME` environment variable).
    pub helios_home: PathBuf,

    /// Directory where Helios writes log files (defaults to `$HELIOS_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// Path to the `helios-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `helios-linux-sandbox`.
    ...
    /// Base URL for requests to ChatGPT (as opposed to the Phenotype API).
    ...
    /// When `true`, checks for Helios updates on startup and surfaces update prompts.
    /// Set to `false` only if your Helios updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Helios product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Helios product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn helios_home(mut self, helios_home: PathBuf) -> Self {
        self.helios_home = Some(helios_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let helios_home = helios_home.map_or_else(find_helios_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let helios_home = find_helios_home()?;
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &helios_home)?;
    ...
    /// in [ConfigToml], such as `cwd` and `helios_linux_sandbox_exe`.
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, helios_home).map_err(|e| {
...
/// Patch `HELIOS_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(helios_home: &Path, provider: &str) -> std::io::Result<()> {
...
pub struct ConfigToml {
    ...
    /// sanctioned by Helios will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/codex/blob/main/helios-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Helios home directory.
    ...
    /// Directory where Helios writes log files, for example `helios-tui.log`.
    /// Defaults to `$HELIOS_HOME/log`.
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, helios_home, config_layer_stack)
    }
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&helios_home));
    ...
    fn load_instructions(helios_dir: Option<&Path>) -> Option<String> {
        let base = helios_dir?;
...
pub fn find_helios_home() -> std::io::Result<PathBuf> {
    helios_utils_home_dir::find_helios_home()
}

/// Returns the path to the folder where Helios logs are stored. Does not verify
...
"#,
    ...
    async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
        ...
        std::fs::write(
            helios_home.path().join(CONFIG_TOML_FILE),
            format!(
                r#"
profile = "global"
...
"#,
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let helios_home = TempDir::new()?;
        let managed_path = helios_home.path().join("managed_config.toml");
        let config_path = helios_home.path().join(CONFIG_TOML_FILE);
        ...
        let cwd = AbsolutePathBuf::try_from(helios_home.path())?;
        let config_layer_stack = load_config_layers_state(
            helios_home.path(),
            Some(cwd),
            &Vec::new(),
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(helios_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(helios_home.path()).await?;
...
"#,
        ...
        let err = load_global_mcp_servers(helios_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/helios-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/helios-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/helios-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(helios_home.path())
            .set_model(Some("gpt-5.1-codex"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(helios_home.path().join(CONFIG_TOML_FILE)).await?;
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn helios_home(&self) -> PathBuf {
            self.helios_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let helios_home = TempDir::new()?;
        let workspace = helios_home.path().join("workspace");
    ...
    fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
        let helios_home = TempDir::new()?;
        let missing_path = helios_home.path().join("agents").join("researcher.toml");
        let cfg = ConfigToml {
    ...
    async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
        let helios_home = TempDir::new()?;
        let role_config_path = helios_home.path().join("agents").join("researcher.toml");
...
"#,
        ...
        let config = ConfigBuilder::default()
            .helios_home(helios_home.path().to_path_buf())
            .fallback_cwd(Some(helios_home.path().to_path_buf()))
            .build()
            .await?;
        assert_eq!(
            config
    ...
    fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
        let helios_home = TempDir::new()?;
        let catalog_path = helios_home.path().join("catalog.json");
...
"#;
        ...
        let helios_home_temp_dir = TempDir::new().unwrap();

        let openai_custom_provider = ModelProviderInfo {
            name: "Phenotype custom".to_string(),
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            helios_home: helios_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                helios_home: fixture.helios_home(),
                log_dir: fixture.helios_home().join("log"),
                config_layer_stack: Default::default(),
                startup_warnings: Vec::new(),
                history: History::default(),
                ephemeral: false,
                file_opener: UriBasedFileOpener::VsCode,
                helios_linux_sandbox_exe: None,
...
"#;
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        ...
        let result = set_default_oss_provider(helios_home, "invalid_provider");
    ...
    fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
        ...
        let result = set_default_oss_provider(helios_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
        assert!(
