...
use helios_app_server_protocol::Config as ApiConfig;
use helios_app_server_protocol::ConfigBatchWriteParams;
use helios_app_server_protocol::ConfigLayerMetadata;
use helios_app_server_protocol::ConfigLayerSource;
use helios_app_server_protocol::ConfigReadParams;
use helios_app_server_protocol::ConfigReadResponse;
use helios_app_server_protocol::ConfigValueWriteParams;
use helios_app_server_protocol::ConfigWriteErrorCode;
use helios_app_server_protocol::ConfigWriteResponse;
use helios_app_server_protocol::MergeStrategy;
use helios_app_server_protocol::OverriddenMetadata;
use helios_app_server_protocol::WriteStatus;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(helios_home: PathBuf) -> Self {
        Self {
            helios_home,
        ...
        let layers = match params.cwd.as_deref() {
            Some(cwd) => {
                ...
                crate::config::ConfigBuilder::default()
                    .helios_home(self.helios_home.clone())
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<helios_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.helios_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.helios_home)
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_helios_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_helios_folder.display(),
        ),
...
mod tests {
    use super::*;
    use anyhow::Result;
    use helios_app_server_protocol::AppConfig;
    use helios_app_server_protocol::AppToolApproval;
    use helios_app_server_protocol::AppsConfig;
...
"#;
    ...
    async fn write_value_preserves_comments_and_order() -> Result<()> {
        ...
        let original = r#"# Helios user configuration
model = "gpt-5"
...
"#;
        ...
        let expected = r#"# Helios user configuration
model = "gpt-5"
approval_policy = "on-request"
