...
impl ShellSnapshot {
    ...
    async fn try_new(
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&helios_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            helios_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(helios_home: &Path, active_session_id: ThreadId) -> Result<()> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(helios_home, session_id).await?;
...
mod tests {
    ...
    fn bash_snapshot_filters_invalid_exports() -> Result<()> {
        let output = Command::new("/bin/bash")
            ...
            .env("VALID_NAME", "ok")
            .env("PWD", "/tmp/stale")
            .env("NEXTEST_BIN_EXE_helios-write-config-schema", "/path/to/bin")
            .env("BAD-NAME", "broken")
            .output()?;
        ...
        assert!(!stdout.contains("NEXTEST_BIN_EXE_helios-write-config-schema"));
    ...
    async fn write_rollout_stub(helios_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
        let dir = helios_home
            .join("sessions")
            .join("2025")
            .join("01")
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(helios_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(helios_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(helios_home, active_session).await?;
