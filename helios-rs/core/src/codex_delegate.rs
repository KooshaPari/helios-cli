...
pub(crate) async fn run_helios_thread_interactive(
    config: Config,
    ...
    let helios_for_events = Arc::clone(&codex);
    ...
    let helios_for_ops = Arc::clone(&codex);
    tokio::spawn(async move {
...
pub(crate) async fn run_helios_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_helios_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn await_approval_with_cancel<F>(
    ...
    approval_id: &str,
    cancel_token: &CancellationToken,
) -> helios_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = helios_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(approval_id, helios_protocol::protocol::ReviewDecision::Abort)
                .await;
            helios_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
