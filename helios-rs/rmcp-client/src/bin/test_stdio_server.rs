...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Helios TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Helios TUI):
    /// - Build this binary: `cargo build -p helios-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `codex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Helios TUI, ask it to call:
...
/// Scenarios for `image_scenario`, intended to exercise Helios TUI handling of MCP image outputs.
///
