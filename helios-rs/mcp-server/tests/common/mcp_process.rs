...
impl McpProcess {
    pub async fn new(helios_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(helios_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = helios_utils_cargo_bin::cargo_bin("helios-mcp-server")
            .context("should find binary for helios-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("helios-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = helios_core::default_client::originator().value;
    ...
    pub async fn send_helios_tool_call(
        &mut self,
        params: CodexToolCallParam,
    ) -> anyhow::Result<i64> {
        let helios_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "codex".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(helios_tool_call_params)?),
        )
        .await
    }
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `helios-mcp-server` child process.
        //
