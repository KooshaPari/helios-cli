//! Asynchronous worker that executes a **Helios** tool-call inside a spawned
...
use helios_core::config::Config as CodexConfig;
...
/// To adhere to MCP `tools/call` response format, include the Helios
...
/// Run a complete Helios session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_helios_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!("Failed to start Helios session: {e}"))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Helios submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_helios_uuid.lock().await.remove(&id);
        return;
    }

    run_helios_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_helios_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_helios_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<CodexThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(helios_event_to_notification(&event)) above has
