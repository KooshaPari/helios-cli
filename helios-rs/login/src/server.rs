                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Helios failed.",
                            io::ErrorKind::Other,
...
/// Returns true when the OAuth callback represents a missing Helios entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_helios_entitlement_error(error_code, error_description) {
        return "Helios is not enabled for your workspace. Contact your workspace administrator to request access to Helios.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_helios_entitlement_error(code, error_description) {
            (
                "You do not have access to Helios".to_string(),
                "This account is not currently authorized to use Helios in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Helios.".to_string(),
                "Contact your workspace administrator to get access to Helios, then return to Helios and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Helios to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
    template
