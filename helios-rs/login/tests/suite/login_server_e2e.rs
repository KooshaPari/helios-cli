...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = helios_home.clone();

    let opts = ServerOptions {
        helios_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: helios_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = helios_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_helios_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let helios_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_helios_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    ...
    assert!(
        !body.contains("missing_helios_entitlement"),
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_helios_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        helios_home: first_helios_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: helios_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_helios_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        helios_home: second_helios_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: helios_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
