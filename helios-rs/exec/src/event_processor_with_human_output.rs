...
use helios_protocol::num_format::format_with_separators;
...
use helios_utils_elapsed::format_elapsed;
...
use helios_utils_sandbox_summary::create_config_summary_entries;
...
pub(crate) struct EventProcessorWithHumanOutput {
    ...
    last_total_token_usage: Option<helios_protocol::protocol::TokenUsageInfo>,
...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn print_config_summary(
        ...
        ts_msg!(
            self,
            "Phenotype Helios v{} (research preview)\n--------",
            VERSION
        );
        ...
        match msg {
            ...
            EventMsg::McpStartupUpdate(update) => {
                let status_text = match update.status {
                    helios_protocol::protocol::McpStartupStatus::Starting => "starting".to_string(),
                    helios_protocol::protocol::McpStartupStatus::Ready => "ready".to_string(),
                    helios_protocol::protocol::McpStartupStatus::Cancelled => {
                        "cancelled".to_string()
                    }
                    helios_protocol::protocol::McpStartupStatus::Failed { ref error } => {
                        format!("failed: {error}")
                    }
                };
                ts_msg!(
...
fn format_receiver_list(ids: &[helios_protocol::ThreadId]) -> String {
    if ids.is_empty() {
