...
use helios_protocol::protocol::ContextCompactedEvent;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `helios-rs/core/rollout/policy.rs`.
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            helios_protocol::items::TurnItem::Plan(plan) => {
            ...
            helios_protocol::items::TurnItem::UserMessage(_)
            | helios_protocol::items::TurnItem::AgentMessage(_)
            | helios_protocol::items::TurnItem::Reasoning(_)
            | helios_protocol::items::TurnItem::WebSearch(_)
            | helios_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &helios_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &helios_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &helios_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &helios_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &helios_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &helios_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &helios_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &helios_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &helios_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &helios_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &helios_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            helios_error_info: payload.helios_error_info.clone().map(Into::into),
...
pub fn convert_patch_changes(
    changes: &HashMap<std::path::PathBuf, helios_protocol::protocol::FileChange>,
...
fn map_patch_change_kind(change: &helios_protocol::protocol::FileChange) -> PatchChangeKind {
    match change {
...
fn format_file_change_diff(change: &helios_protocol::protocol::FileChange) -> String {
    match change {
        helios_protocol::protocol::FileChange::Add { content } => content.clone(),
        helios_protocol::protocol::FileChange::Delete { content } => content.clone(),
        helios_protocol::protocol::FileChange::Update {
            unified_diff,
            move_path,
        } => {
...
mod tests {
    ...
    use helios_protocol::items::UserMessageItem as CoreUserMessageItem;
    use helios_protocol::models::MessagePhase as CoreMessagePhase;
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    helios_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(helios_protocol::protocol::CollabResumeEndEvent {
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                helios_error_info: Some(CodexErrorInfo::ResponseStreamDisconnected {
