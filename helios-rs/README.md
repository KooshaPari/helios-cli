...
### Release build + local registration

For local development and true fork parity, build and register the release binary directly:

```sh
cd /path/to/heliosCLI-wtrees/stash-resolution/helios-rs
cargo build --manifest-path Cargo.toml -p helios-cli --release
./scripts/register-helios-release-bins.sh
```

Then point `helios` and `helios-dev` at the release binary:

```sh
./scripts/register-helios-release-bins.sh
~/.local/bin/helios --version
~/.local/bin/helios-dev --version
```

If the build succeeds but symlinks are stale, rerun the script after each rebuild:
