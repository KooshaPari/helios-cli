use crate::helios_message_processor::ApiVersion;
use crate::helios_message_processor::read_rollout_items_from_rollout;
use crate::helios_message_processor::read_summary_from_rollout;
use crate::helios_message_processor::summary_to_thread;
...
use helios_app_server_protocol::AccountRateLimitsUpdatedNotification;
use helios_app_server_protocol::AgentMessageDeltaNotification;
use helios_app_server_protocol::ApplyPatchApprovalParams;
use helios_app_server_protocol::ApplyPatchApprovalResponse;
use helios_app_server_protocol::CodexErrorInfo as V2CodexErrorInfo;
use helios_app_server_protocol::CollabAgentState as V2CollabAgentStatus;
use helios_app_server_protocol::CollabAgentTool;
use helios_app_server_protocol::CollabAgentToolCallStatus as V2CollabToolCallStatus;
use helios_app_server_protocol::CommandAction as V2ParsedCommand;
use helios_app_server_protocol::CommandExecutionApprovalDecision;
use helios_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
use helios_app_server_protocol::ContextCompactedNotification;
use helios_app_server_protocol::DeprecationNoticeNotification;
use helios_app_server_protocol::DynamicToolCallParams;
use helios_app_server_protocol::ErrorNotification;
use helios_app_server_protocol::ExecCommandApprovalParams;
use helios_app_server_protocol::ExecCommandApprovalResponse;
use helios_app_server_protocol::ExecPolicyAmendment as V2ExecPolicyAmendment;
...
use helios_app_server_protocol::FileUpdateChange;
...
use helios_app_server_protocol::McpToolCallError;
use helios_app_server_protocol::McpToolCallResult;
use helios_app_server_protocol::McpToolCallStatus;
use helios_app_server_protocol::ModelReroutedNotification;
use helios_app_server_protocol::NetworkApprovalContext as V2NetworkApprovalContext;
use helios_app_server_protocol::PatchApplyStatus;
use helios_app_server_protocol::PlanDeltaNotification;
use helios_app_server_protocol::RawResponseItemCompletedNotification;
use helios_app_server_protocol::ReasoningSummaryPartAddedNotification;
use helios_app_server_protocol::ReasoningSummaryTextDeltaNotification;
use helios_app_server_protocol::ReasoningTextDeltaNotification;
use helios_app_server_protocol::ServerNotification;
use helios_app_server_protocol::ServerRequestPayload;
use helios_app_server_protocol::TerminalInteractionNotification;
use helios_app_server_protocol::ThreadItem;
use helios_app_server_protocol::ThreadNameUpdatedNotification;
use helios_app_server_protocol::ThreadRollbackResponse;
use helios_app_server_protocol::ThreadTokenUsage;
use helios_app_server_protocol::ThreadTokenUsageUpdatedNotification;
use helios_app_server_protocol::ToolRequestUserInputOption;
use helios_app_server_protocol::ToolRequestUserInputParams;
use helios_app_server_protocol::ToolRequestUserInputQuestion;
use helios_app_server_protocol::ToolRequestUserInputResponse;
use helios_app_server_protocol::Turn;
use helios_app_server_protocol::TurnCompletedNotification;
use helios_app_server_protocol::TurnDiffUpdatedNotification;
use helios_app_server_protocol::TurnError;
use helios_app_server_protocol::TurnInterruptResponse;
use helios_app_server_protocol::TurnPlanStep;
use helios_app_server_protocol::TurnPlanUpdatedNotification;
use helios_app_server_protocol::TurnStatus;
use helios_app_server_protocol::build_turns_from_rollout_items;
use helios_app_server_protocol::convert_patch_changes;
use helios_core::CodexThread;
use helios_core::ThreadManager;
use helios_core::find_thread_name_by_id;
use helios_core::review_format::format_review_findings_block;
use helios_core::review_prompts;
...
use helios_protocol::protocol::TokenCountEvent;
use helios_protocol::protocol::TurnDiffEvent;
use helios_protocol::request_user_input::RequestUserInputAnswer as CoreRequestUserInputAnswer;
use helios_protocol::request_user_input::RequestUserInputResponse as CoreRequestUserInputResponse;
use helios_shell_command::parse_command::shlex_join;
    ...
    match msg {
        ...
        EventMsg::CollabAgentSpawnEnd(end_event) => {
            ...
            let status = match &end_event.status {
                helios_protocol::protocol::AgentStatus::Errored(_)
                | helios_protocol::protocol::AgentStatus::NotFound => V2CollabToolCallStatus::Failed,
        ...
        EventMsg::CollabWaitingEnd(end_event) => {
            let status = if end_event.statuses.values().any(|status| {
                matches!(
                    status,
                    helios_protocol::protocol::AgentStatus::Errored(_)
                        | helios_protocol::protocol::AgentStatus::NotFound
                )
            }) {
        ...
        EventMsg::AgentMessageContentDelta(event) => {
            let helios_protocol::protocol::AgentMessageContentDeltaEvent { item_id, delta, .. } =
                event;
        ...
        EventMsg::Error(ev) => {
            ...
            let helios_error_info = ev.helios_error_info.clone();
            ...
            let turn_error = TurnError {
                message: ev.message,
                helios_error_info: ev.helios_error_info.map(V2CodexErrorInfo::from),
...
async fn maybe_emit_raw_response_item_completed(
    ...
    conversation_id: ThreadId,
    turn_id: &str,
    item: helios_protocol::models::ResponseItem,
    outgoing: &ThreadScopedOutgoingMessageSender,
) {
...
fn collab_resume_begin_item(
    begin_event: helios_protocol::protocol::CollabResumeBeginEvent,
) -> ThreadItem {
...
fn collab_resume_end_item(end_event: helios_protocol::protocol::CollabResumeEndEvent) -> ThreadItem {
    let status = match &end_event.status {
        helios_protocol::protocol::AgentStatus::Errored(_)
        | helios_protocol::protocol::AgentStatus::NotFound => V2CollabToolCallStatus::Failed,
        _ => V2CollabToolCallStatus::Completed,
...
mod tests {
    ...
    use anyhow::anyhow;
    use anyhow::bail;
    use helios_app_server_protocol::TurnPlanStepStatus;
    ...
    fn collab_resume_end_maps_to_item_completed_resume_agent() {
        let event = CollabResumeEndEvent {
            ...
            receiver_agent_nickname: None,
            receiver_agent_role: None,
            status: helios_protocol::protocol::AgentStatus::NotFound,
        };
        ...
        let expected = ThreadItem::CollabAgentToolCall {
            ...
            agents_states: [(
                receiver_id,
                V2CollabAgentStatus::from(helios_protocol::protocol::AgentStatus::NotFound),
            )]
            .into_iter()
            .collect(),
        };
    ...
    async fn test_handle_error_records_message() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                helios_error_info: Some(V2CodexErrorInfo::InternalServerError),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_failed_with_error() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                helios_error_info: Some(V2CodexErrorInfo::Other),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_error_multiple_turns() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                helios_error_info: Some(V2CodexErrorInfo::BadRequest),
                additional_details: None,
            },
            &thread_state,
        )
