...
const HELIOS_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Helios, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let helios_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(helios_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(helios_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(helios_home.path()).await?;
...
async fn thread_resume_rejoins_running_thread_even_with_override_mismatch() -> Result<()> {
    ...
    let ThreadResumeResponse { thread, model, .. } =
        to_response::<ThreadResumeResponse>(resume_resp)?;
    assert_eq!(model, "gpt-5.1-helios-max");
    assert_eq!(
        thread.status,
        ThreadStatus::Active {
            active_flags: vec![],
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        thread_id,
        rollout_file_path,
    } = start_materialized_thread_and_restart(helios_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let helios_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(helios_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(helios_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(helios_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(helios_home).await?;
    ...
    let mut second_mcp = McpProcess::new(helios_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(HELIOS_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
command = "helios-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
fn setup_rollout_fixture(helios_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(helios_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(helios_home, filename_ts, &conversation_id);
    set_rollout_mtime(rollout_file_path.as_path(), expected_updated_at_rfc3339)?;
    let before_modified = std::fs::metadata(&rollout_file_path)?.modified()?;
    let expected_updated_at = chrono::DateTime::parse_from_rfc3339(expected_updated_at_rfc3339)?
        .with_timezone(&Utc)
