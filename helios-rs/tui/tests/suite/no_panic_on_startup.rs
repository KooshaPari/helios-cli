...
async fn malformed_rules_should_not_panic() -> anyhow::Result<()> {
    // run_helios_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
...
async fn run_helios_cli(
    helios_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<CodexCliOutput> {
    let helios_cli = helios_utils_cargo_bin::cargo_bin("codex")?;
    let mut env = HashMap::new();
    env.insert(
        "HELIOS_HOME".to_string(),
        helios_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = helios_utils_pty::spawn_pty_process(
        helios_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
