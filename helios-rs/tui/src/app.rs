...
use helios_core::models_manager::model_presets::HIDE_GPT_5_1_HELIOS_MAX_MIGRATION_PROMPT_CONFIG;
use helios_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
#[cfg(target_os = "windows")]
...
use helios_protocol::protocol::FinalOutput;
...
fn session_summary(
    ...
    let resume_command = helios_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    {
        let ConfigLayerSource::Project { dot_helios_folder } = &layer.name else {
            continue;
        };
...
fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_HELIOS_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_helios_max_migration_prompt
            .unwrap_or(false),
...
impl App {
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .helios_home(self.config.helios_home.clone())
    ...
    fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, HELIOS_CLI_VERSION)
    }
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let helios_op_tx = if let Some(thread) = live_thread {
            crate::chatwidget::spawn_op_forwarder(thread)
        } else {
            let (tx, _rx) = unbounded_channel();
            tx
        };
        self.chat_widget = ChatWidget::new_with_op_sender(init, helios_op_tx);
    ...
    async fn drain_active_thread_events(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        loop {
            match rx.try_recv() {
                Ok(event) => self.handle_helios_event_now(event),
    ...
    fn replay_thread_snapshot(&mut self, snapshot: ThreadEventSnapshot) {
        if let Some(event) = snapshot.session_configured {
            self.handle_helios_event_replay(event);
        }
    ...
    pub async fn run(
        ...
        let otel_manager = OtelManager::new(
            ...
            auth_mode,
            helios_core::default_client::originator().value,
            config.otel.log_user_prompt,
            helios_core::terminal::user_agent(),
            SessionSource::Cli,
        );
        if config
            .tui_status_line
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.helios_home.clone();
        ...
        match event {
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = helios_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    helios_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                otel_manager.counter(
                                    helios_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    1,
                                    &tags,
            ...
            AppEvent::BeginWindowsSandboxLegacySetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.chat_widget.show_windows_sandbox_setup_status();
                    tokio::task::spawn_blocking(move || {
                        if let Err(err) = helios_core::windows_sandbox::run_legacy_setup_preflight(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match helios_core::windows_sandbox_read_grants::grant_read_root_non_elevated(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder::new(&self.config.helios_home)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.helios_home.clone();
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                let mut builder = ConfigEditsBuilder::new(&self.config.helios_home)
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.helios_home)
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let edit = helios_core::config::edit::status_line_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.helios_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                let edit = helios_core::config::edit::syntax_theme_edit(&name);
                let apply_result = ConfigEditsBuilder::new(&self.config.helios_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn handle_helios_event_now(&mut self, event: Event) {
        ...
        self.chat_widget.handle_helios_event(event);
    ...
    fn handle_helios_event_replay(&mut self, event: Event) {
        self.chat_widget.handle_helios_event_replay(event);
    }
    ...
    async fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
        ...
        self.handle_helios_event_now(event);
        if self.backtrack_render_pending {
            tui.frame_requester().schedule_frame();
        }
        Ok(())
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("helios-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> helios_protocol::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
    ...
    fn spawn_world_writable_scan(
        ...
        tokio::task::spawn_blocking(move || {
            let result = helios_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    async fn make_test_app() -> App {
        ...
        let server = Arc::new(
            helios_core::test_support::thread_manager_with_models_provider(
        ...
        App {
            ...
            backtrack: BacktrackState::default(),
            backtrack_render_pending: false,
            feedback: helios_feedback::CodexFeedback::new(),
    ...
    fn test_otel_manager(config: &Config, model: &str) -> OtelManager {
        let model_info = helios_core::test_support::construct_model_info_offline(model, config);
        OtelManager::new(
            ThreadId::new(),
            model,
    ...
    fn all_model_presets() -> Vec<ModelPreset> {
        helios_core::test_support::all_model_presets().clone()
    }
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        let helios_home = tempdir().expect("temp codex home");
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        app.config.helios_home = helios_home.path().to_path_buf();
        ...
        ConfigEditsBuilder::new(&app.config.helios_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
        ...
        app.chat_widget.handle_helios_event(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id: base_id,
                forked_from_id: None,
    ...
    async fn replayed_initial_messages_apply_rollback_in_queue_order() {
        ...
        app.handle_helios_event_replay(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id,
                forked_from_id: None,
    ...
    async fn live_rollback_during_replay_is_applied_in_app_event_order() {
        ...
        app.handle_helios_event_now(Event {
