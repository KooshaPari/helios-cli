...
// The standalone `helios-tui` binary prints a short help message before the
...
use helios_core::terminal::Multiplexer;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = helios_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let helios_home = match find_helios_home() {
        Ok(helios_home) => helios_home.to_path_buf(),
        Err(err) => {
    ...
    if let Err(err) =
        helios_core::personality_migration::maybe_migrate_personality(&helios_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&helios_home).await?;
    ...
    let log_dir = helios_core::config::log_dir(&config)?;
    ...
    let log_file = log_file_opts.open(log_dir.join("helios-tui.log"))?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("helios_core=info,helios_tui=info,helios_rmcp_client=info")
        })
    };
    ...
    let feedback = helios_feedback::CodexFeedback::new();
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        helios_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, true)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
            #[allow(clippy::print_stderr)]
    ...
    let log_db_layer = helios_core::state_db::get_state_db(&config, None)
        .await
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: helios_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let auth_manager = AuthManager::shared(
        initial_config.helios_home.clone(),
        false,
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.helios_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.helios_home, id_str).await?
            };
            match path {
...
mod tests {
    ...
    use helios_core::config::ConfigBuilder;
    use helios_core::config::ConfigOverrides;
    use helios_core::config::ProjectConfig;
    use helios_protocol::protocol::AskForApproval;
    use helios_protocol::protocol::RolloutItem;
    use helios_protocol::protocol::RolloutLine;
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .helios_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let helios_home = temp_dir.path().to_path_buf();
