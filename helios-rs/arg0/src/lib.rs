...
const LINUX_SANDBOX_ARG0: &str = "helios-linux-sandbox";
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == LINUX_SANDBOX_ARG0 {
        ...
        helios_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == HELIOS_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                match helios_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
            None => {
                eprintln!("Error: {HELIOS_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    match prepend_path_entry_for_helios_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Helios will proceed successfully even if
...
/// While we want to deploy the Helios CLI as a single executable for simplicity,
...
/// `helios-linux-sandbox` we *directly* execute
/// [`helios_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
...
///     error. Note that `main_fn` receives `helios_linux_sandbox_exe:
///     Option<PathBuf>`, as an argument, which is generally needed as part of
///     constructing [`helios_core::config::Config`].
///
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    runtime.block_on(async move {
        let helios_linux_sandbox_exe: Option<PathBuf> = if cfg!(target_os = "linux") {
            std::env::current_exe().ok()
        } else {
            None
        };

        main_fn(helios_linux_sandbox_exe).await
    })
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "HELIOS_";

/// Load env vars from ~/.codex/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `HELIOS_`.
fn load_dotenv() {
    if let Ok(helios_home) = find_helios_home()
        && let Ok(iter) = dotenvy::from_path_iter(helios_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `HELIOS_` keys.
...
///   with the "secret" --helios-run-as-apply-patch flag.
///
...
pub fn prepend_path_entry_for_helios_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let helios_home = find_helios_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if helios_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (helios_home: {helios_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a HELIOS_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = helios_home.join("tmp").join("arg0");
