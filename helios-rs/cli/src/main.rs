...
use helios_cli::LandlockCommand;
use helios_cli::SeatbeltCommand;
use helios_cli::WindowsCommand;
use helios_cli::login::read_api_key_from_stdin;
use helios_cli::login::run_login_status;
use helios_cli::login::run_login_with_api_key;
use helios_cli::login::run_login_with_chatgpt;
use helios_cli::login::run_login_with_device_code;
use helios_cli::login::run_logout;
use helios_cloud_tasks::Cli as CloudTasksCli;
use helios_exec::Cli as ExecCli;
use helios_exec::Command as ExecCommand;
use helios_exec::ReviewArgs;
use helios_execpolicy::ExecPolicyCheckCommand;
use helios_responses_api_proxy::Args as ResponsesApiProxyArgs;
use helios_tui::AppExitInfo;
use helios_tui::Cli as TuiCli;
use helios_tui::ExitReason;
use helios_tui::update_action::UpdateAction;
...
#[clap(
    ...
    // `helios-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `helios` command name that users run.
    bin_name = "helios",
    override_usage = "helios [OPTIONS] [PROMPT]\n       helios [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = helios_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    let mut lines = vec![format!(
        "{}",
        helios_protocol::protocol::FinalOutput::from(token_usage)
    )];

    if let Some(resume_cmd) =
        helios_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
fn stage_str(stage: helios_core::features::Stage) -> &'static str {
    use helios_core::features::Stage;
    match stage {
    ...
    match subcommand {
        ...
        Some(Subcommand::AppServer(app_server_cli)) => match app_server_cli.subcommand {
            ...
            Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                let options = helios_app_server_protocol::GenerateTsOptions {
                    experimental_api: gen_cli.experimental,
                    ..Default::default()
                };
                helios_app_server_protocol::generate_ts_with_options(
                    &gen_cli.out_dir,
            ...
            Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                helios_app_server_protocol::generate_json_with_experimental(
                    &gen_cli.out_dir,
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            tokio::task::spawn_blocking(move || helios_responses_api_proxy::run_main(args))
                .await??;
        }
        Some(Subcommand::StdioToUds(cmd)) => {
            let socket_path = cmd.socket_path;
            tokio::task::spawn_blocking(move || helios_stdio_to_uds::run(socket_path.as_path()))
                .await??;
        }
        Some(Subcommand::Features(FeaturesCli { sub })) => match sub {
            FeaturesSubcommand::List => {
                ...
                .await?;
                let mut rows = Vec::with_capacity(helios_core::features::FEATURES.len());
                let mut name_width = 0;
                let mut stage_width = 0;
                for def in helios_core::features::FEATURES.iter() {
                    let name = def.key;
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&helios_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let Some(spec) = helios_core::features::FEATURES
        .iter()
    ...
    let config_path = helios_home.join(helios_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_interactive_tui(
    ...
    let terminal_info = helios_core::terminal::terminal_info();
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(helios_exec::Command::Resume(args)) = exec.command else {
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(helios_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(helios_utils_cli::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            helios_app_server::AppServerTransport::WebSocket {
